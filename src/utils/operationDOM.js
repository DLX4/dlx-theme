class OpeartionDOM {
  ...
  outerWidth(bool) {
    const margin =
      this._styleToNumber("marginRight") + this._styleToNumber("marginLeft");
    const padding =
      this._styleToNumber("paddingRight") + this._styleToNumber("paddingLeft");
    const border =
      this._styleToNumber("borderRight") + this._styleToNumber("borderLeft");
    return bool
      ? this.$el.offsetWidth + margin + padding + border
      : this.$el.offsetWidth + padding + border;
  }

  outerHeight(bool) {
    const margin =
      this._styleToNumber("marginTop") + this._styleToNumber("marginBottom");
    const padding =
      this._styleToNumber("paddingTop") + this._styleToNumber("paddingBottom");
    const border =
      this._styleToNumber("borderTop") + this._styleToNumber("borderBottom");
    return bool
      ? this.$el.offsetHeight + margin + padding + border
      : this.$el.offsetHeight + padding + border;
  }
