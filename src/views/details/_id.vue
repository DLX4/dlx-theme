<template>
  <section class="container">
    ...
    <article class="section article">
      ...
      <div class="other-info">
        ...
        <p>
          ...
          <span class="m-l-5px">
            <x-icon type="icon-message-f"></x-icon
    ...
    <div v-if="info.isOpenArticleCopyright" class="section copyright">
      ...
      <p class="m-t-10px">
        <strong>本文地址: </strong><a :href="fullPath">{{ fullPath }}</a>
      </p>
    </div>
    <!-- 文章内容结束 -->
    ...
    <div class="section operation">
      ...
      <ul class="opinion">
        <li
          ...
          :key="key"
          @click="_updateOpinion(key)"
        >
          <span class="block total">
            <x-icon type="icon-loading" v-show="item.isShowLaoding"></x-icon>
            {{ item.data }}人
          </span>
      ...
      <div class="tag-wrap text-center">
        <x-icon
          type="icon-tag"
          v-show="detail.articleInfor.tags.length"
        ></x-icon>
        <span
          v-for="(item, index) in detail.articleInfor.tags"
          :key="item.key"
          v-html="
            index === detail.articleInfor.tags.length - 1
              ? item.name
              : `${item.name}、`
          "
        ></span>
        ...
        <div class="prev">
          ...
          <p v-else>
            上一篇：<router-link
              ...
              >{{ detail.articleInfor.prevLink.post_title }}</router-link
            >
          </p>
        </div>
        <div class="next">
          ...
          <p v-else>
            下一篇：<router-link
              ...
              >{{ detail.articleInfor.nextLink.post_title }}</router-link
            >
          </p>
        </div>
      </div>
        ...
        <ul class="author-link">
          ...
          <template v-for="(item, key) in authorOtherInfo">
            ...
            <li v-else :key="item.key" class="list">
              <a
                :href="key == 'email' ? `mailto:${item.url}` : item.url"
                target="_blank"
              >
                <svg-icon :iconName="item.icon"></svg-icon>
              </a>
...
export default {
  ...
  beforeDestroy() {
    document
      .querySelectorAll(".prism-previewer")
