<template>
  <div class="comments-wrap">
    <!-- 发表评论 -->
    <div
      v-if="commentStatus === 'closed'"
      class="comment-closed text-center f-s-large"
    >
      评论已关闭
    </div>
    <div v-else-if="commentStatus === 'open'" class="comment-from">
      ...
      <p class="comment-sub-title">
        电子邮件地址不会被公开。 必填项已用<i class="c-red">*</i>标注
      </p>
      <!-- 评论其他功能 -->
      <div class="comment-other">
        ...
        <!-- 表情 -->
        <expression
          :is-show="expression.isShow"
          @on-change="_insertExpression"
        />
      </div>
      <!-- 评论输入框 -->
      ...
      <div class="box">
        <div class="comment-inp comment-from-author">
          ...
          />
          <span v-show="author.validate" class="comment-tips">{{
            author.msg
          }}</span>
        </div>
        ...
        <div class="comment-inp comment-from-code">
          ...
          <canvas
            ...
            @click="_randomCode"
          ></canvas>
          <span v-show="imgCode.validate" class="comment-tips">{{
            imgCode.msg
          }}</span>
        </div>
      </div>
    ...
    <ul class="comment-list-wrap">
      ...
      >
        ...
        <div class="list-header">
          ...
          <a :href="item.author_url" target="_blank" class="author">{{
            item.author_name
          }}</a>
          <!-- 评论者等级 -->
          <span
            ...
            :class="[
              item.userAgentInfo.vipStyle.style,
              item.userAgentInfo.vipStyle.level
            ]"
          ...
          <p class="inline-block system-wrap">
            ...
            <span
              ...
              class="browser-info"
              :class="item.userAgentInfo.userAgent.browserName.toLowerCase()"
            >
              {{ item.userAgentInfo.userAgent.browserName.replace("-", " ") }}
              {{ item.userAgentInfo.userAgent.browserVersion }}
            </span>
            <span v-else class="browser-info">Unkonw</span>
            <!-- 系统logo -->
            <span class="system-info">{{
              item.userAgentInfo.userAgent.system &&
                item.userAgentInfo.userAgent.system.replace(/_/g, ".")
            }}</span>
          </p>
        ...
        <div class="list-footer">
          ...
          <div class="opinion">
            <span
              class="opinion-btn"
              @click="_updateCommentOpinion(item.id, 'good', index)"
            >
              <x-icon type="icon-good"></x-icon> {{ item.meta.opinion.good }}
            </span>
            <span
              class="opinion-btn"
              @click="_updateCommentOpinion(item.id, 'bad', index)"
            >
